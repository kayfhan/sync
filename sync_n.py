...
def get_ch_column_type(db, table, conf):
    ...
    try:
        ...
        sql = "select name,type from system.columns where database='{0}' and table='{1}'".format(db, 's_' + table)
...
def insert_update(tmp_data, pk_dict):
    ...
    insert_sql = "INSERT INTO {0}.{1} VALUES ".format(schema, 's_' + table)
...
def event_primary_key(schema, table, tmp_data, pk_dict):
    ...
    if value_num == 1:
        del_sql = "alter table {0}.{1} delete where {2}".format(schema, 's_' + table, tmp_sql)
