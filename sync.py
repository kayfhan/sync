...
class my_db():
    ...
    def get_pri(self, db, table):
        # 获取主键或者唯一键信息
        primary_key = []
        pri_sql = "select COLUMN_NAME from information_schema.COLUMNS where TABLE_SCHEMA='{0}' and TABLE_NAME='{1}' and COLUMN_KEY='PRI'".format(
            db, table)
        uni_sql = "select COLUMN_NAME from information_schema.COLUMNS where TABLE_SCHEMA='{0}' and TABLE_NAME='{1}' and COLUMN_KEY='UNI'".format(
            db, table)
    ...
    def get_unique(self, db, table):
        ...
        uni_sql = "SELECT  COLUMN_NAME  FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA='{0}' \
                 ...
                 WHERE TABLE_SCHEMA='{2}' AND TABLE_NAME='{3}' and CONSTRAINT_TYPE='UNIQUE')".format(db, table, db,
                                                                                                     table)
        try:
            cursor = self.conn.cursor()
    ...
    def check_table_exists(self, db, table):
        sql = "select count(*) as count from information_schema.tables where TABLE_SCHEMA='{0}' and TABLE_NAME='{1}'".format(
            db, table)
        try:
            cursor = self.conn.cursor()
            cursor.execute(sql)
    ...
    def set_log_pos(self, *args):
        try:
            if args[0] == 'slave':
                ...
                self.redis_conn().hmset(self.key, {'master_host': args[1], 'master_port': args[2],
                                                   'relay_master_log_file': args[3], 'exec_master_log_pos': args[4]})
        except Exception as error:
        ...
        for binlogevent in stream:
            for row in binlogevent.rows:
                ...
                if len(event_list) >= insert_nums or (
                        int(time.time()) - event_list[0]['event_unixtime'] >= interval and interval > 0):
...
def insert_update(tmp_data, pk_dict):
    ...
    insert_sql = "INSERT INTO {0}.{1} VALUES ".format('warehouse', 's_' + table)
    exec_sql['del_sql'] = del_sql
    exec_sql['insert_sql'] = insert_sql
    exec_sql['insert_data'] = insert_data
    exec_sql['db_tb'] = "{0}.{1}".format('warehouse', table)
    query_sql = del_sql
...
def event_primary_key(schema, table, tmp_data, pk_dict):
    ...
    if value_num == 1:
        del_sql = "alter table {0}.{1} delete where {2}".format('warehouse', 's_' + table, tmp_sql)
...
def data_to_ck(event, alarm_info, alarm_mail, debug, skip_dmls_all, skip_delete_tb_name, skip_update_tb_name, pk_dict,
               only_schemas, **kwargs):
    client = clickhouse_driver.Client(host=kwargs['host'], port=kwargs['port'], user=kwargs['user'],
    ...
    if len(only_schemas) == 1:
        query_sql = "select count(*) from system.mutations where is_done=0 and database in ('warehouse')"
        query_sql = query_sql.replace(",", '')
        query_schema = str(tuple(only_schemas))
        query_schema = query_schema.replace(',', '')
        mutation_sql = "select count(*) as mutation_faild ,concat(database,'.',table)as db,create_time from system.mutations where is_done=0 and database in ('warehouse') group by db,create_time"
    else:
...
def init_parse():
    ...
    parser.add_argument('-c', '--conf', required=False, default='./metainfo.conf',
                        help='Data synchronization information file')
    parser.add_argument('-d', '--debug', action='store_true', default=False, help='Display SQL information')
    parser.add_argument('-l', '--logtoredis', action='store_true', default=False,
                        help='log position to redis ,default file')
    return parser
